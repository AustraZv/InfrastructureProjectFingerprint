...
(function buildMap() {
  for (var i = 0; i < TRACKER_DB.length; i++) {
    var tracker = TRACKER_DB[i];

    for (var j = 0; j < tracker.domains.length; j++) {
      var domain = tracker.domains[j];
      TRACKER_MAP[domain] = tracker;
    }
  }
})();
